...
1. ‘NOOBS’ 다운로드 https://www.raspberrypi.org/downloads/
...
2.  ‘SD formatter’ 다운로드 &amp; 설치 https://www.sdcard.org/downloads/formatter_4/index.html

3.  SD카드를 노트북에 삽입하고 SD formatter 실행.
...
4. NOOPS zip파일을 풀어 SD카드에 모두 복사
5. RPI에 usb(파워), HDMI( 모니터), 키보드, 마우스, 그리고 SD카드를 끼우고 실행!
6. os 선택하도록 하는데, Raspbian 을 선택. 아래에서 언어는 한글로, 키보드레이아웃은 kr로… ’설치’를 눌러 설치 진행.
...
1.  usb wifi 동글을 꽂는다.
2.  xwindow 우상단에 네트워크 아이콘을 클릭해 AP를 선택하고 비번을 입력하면 끝.
...
4. 한참 기다린 후 업그레이드가 완료되면 다운로드한 업데이트 파일을 지워서 용량을 확보한다.
...
1. 최근의 라즈비안에는 realVNC가 기본탑재되어있다. 새로인 install 할 필요는 없다. 구지 해야한다면
...
<li>키보드와 마우스, 모니터를 다 뽑고 재부팅한다. 노트북에서 잘 컨트롤 됨을 확인한다.
...
<li>마지막으로, ip address는 보통 고정되어있지 않고 부팅할 때마다 바뀌기 때문에, 어느날 갑자기 VNC연결이 안 될 수도 있다. 공유기 설정에서 라즈베리파이(에 연결된 ipTIME usb 동글)에 항상 정해진 ip 주소를 할당토록 설정한다. 방법은 공유기마다 다르겠지만…

<img src="https://cl.ly/pwvV/Image%202018-03-06%20at%205.04.36%20AM.png" width=600>
</p></li>
<li>노트북과 라즈베리파이 사이에 파일교환이 필요한 경우 VNC viewer의 화살표 아이콘을 누르거나, 라즈베리파이의 작업표시줄상 VNC아이콘을 누르면 된다.
